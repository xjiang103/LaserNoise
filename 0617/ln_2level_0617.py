...
def run_job():
    ...
    #f_k=(k+1)*df.
    phase_arr=np.random.uniform(0,np.pi,size=nf)
...
def swp_lw(lw):
    for k in range(nf):
        s_nu_arr[k] = 2*np.sqrt(s_nu((k+0.1)*df,lw)*df)
    res=[]
    ...
    return [meanr0,(np.std(totresult,axis=0))[0],stdp,stdn,r0]

timet=time.time()
swparr=[[1000,1],[1000,1],[1000,1],[10000,2],[1000,2],[100,2]]
for i in range(1):
    tpi=tpi0*swparr[i][1]
    lwset=swparr[i][0]
